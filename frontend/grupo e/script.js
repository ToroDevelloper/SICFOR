...
// ===== FUNCIÓN PARA PROCESAR URLs DE AWS =====
function processResourceUrl(resource) {
    if (!resource.url) return resource.url;
    // Si es una URL de AWS S3, convertir al endpoint del servidor
    if (resource.url && resource.url.includes('amazonaws.com')) {
        // Extraer el Key del archivo desde la URL
        const urlParts = resource.url.split('.amazonaws.com/');
        if (urlParts.length === 2) {
            const key = urlParts[1];
            return `${API_URL}/download-aws/${key}`;
        }
    }
    return resource.url;
}
...
let storageType = 'aws'; // Cambiar a 'aws' por defecto
...
async function loadResources() {
    try {
        ...
        showNotification('Error al conectar con el servidor. Verifica que esté corriendo.', 'error');
    }
}
...
function renderRecentResources() {
    ...
    if (recent.length === 0) {
        container.innerHTML = '<p class="empty-state">No hay recursos agregados aun</p>';
        return;
    }
...
function renderResources(type) {
    ...
    container.innerHTML = filtered.map(resource => {
        const processedUrl = processResourceUrl(resource);
        const isImage = processedUrl && (processedUrl.match(/\.(jpg|jpeg|png|gif|webp|svg)$/i) || resource.url.includes('cloudinary'));
        const isVideo = processedUrl && (processedUrl.match(/\.(mp4|webm|mov|avi|mkv)$/i) || resource.url.includes('cloudinary'));
                                          
        let preview = '';
        if (isImage && processedUrl) {
            preview = `<img src="${processedUrl}" alt="${resource.titulo}" class="resource-preview" onclick="openMediaModal(${resource.id}, 'image')" style="cursor: pointer;">`;
        } else if (isVideo && processedUrl) {
            preview = `
                <div class="resource-preview-video">
                    <video src="${processedUrl}" class="resource-preview" style="max-height: 200px; width: 100%;" controls onclick="event.stopPropagation()" onplay="event.stopPropagation()"></video>
        ...
        return `
            <div class="resource-card">
                ...
                <div class="resource-actions">
                    ${processedUrl ? `<a href="${processedUrl}" target="_blank" class="btn btn-primary btn-icon">Abrir</a>` : ''}
                    ${(isImage || isVideo) && processedUrl ? `<button class="btn btn-secondary btn-icon" onclick="downloadMedia('${processedUrl}', '${resource.titulo}')">Descargar</button>` : ''}
...
async function saveResource(e) {
    ...
    try {
        if (archivoInput.files[0]) {
            ...
            showNotification('⏳ Subiendo archivo a 📦 AWS...', 'info');
            ...
            const response = await fetch(`${API_URL}/upload-aws`, {
                method: 'POST',
                body: formData
            });
            ...
            showNotification('✅ ¡Archivo subido exitosamente a 📦 AWS! Recurso creado.', 'success');
        } else {
...
function openMediaModal(resourceId, type) {
    ...
    if (type === 'image') {
        mediaContainer.innerHTML = `<img src="${processedUrl}" alt="${resource.titulo}" style="max-width: 100%; max-height: 70vh; object-fit: contain;">`;
    } else if (type === 'video') {
...
function cambiarTipoStorage() {
    ...
    const tipoNombre = selectedStorage === 'local' ? '💾 Local' : (selectedStorage === 'aws' ? '📦 AWS' : '☁️ Cloudinary');
...
async function saveCrudResource(e) {
    ...
    try {
        if (editingResourceId) {
            ...
            // Crear nuevo recurso
            if (archivoInput.files[0]) {
                // Subir archivo (local o AWS según selección)
                const endpoint = storageType === 'aws' ? `${API_URL}/upload-aws` : `${API_URL}/upload-local`;
                const tipoDeStorage = storageType === 'aws' ? '📦 AWS' : '💾 Local';
                
                showNotification(`⏳ Subiendo archivo a ${tipoDeStorage}...`, 'info');
                ...
                if (!response.ok) {
                    const error = await response.json();
                    throw new Error(error.error || `Error al subir archivo a ${tipoDeStorage}`);
                }

                const newResource = await response.json();
                resources.push(newResource);
                showNotification(`✅ ¡Archivo subido a ${tipoDeStorage} exitosamente! Recurso creado.`, 'success');
            } else {
                // Crear con URL solamente
                const resourceData = {
                    tipo: tipoSelect.value,
...
async function clearAllData() {
    if (!confirm('⚠️ ¿Estás seguro? Esto eliminará TODOS los recursos. No se puede deshacer.')) {
        return;
    }
